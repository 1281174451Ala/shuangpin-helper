...
pub fn has_accessibility_permission() -> bool {
  ...
  match std::env::current_exe() {
    Ok(path) => println!("[权限诊断] 当前二进制路径: {:?}, 权限状态: {}", path, trusted),
    Err(err) => println!("[权限诊断] 无法获取当前二进制路径: {}, 权限状态: {}", err, trusted),
  }
  trusted
}
...
pub fn is_listening() -> bool {
  listener_slot().lock().unwrap().is_some()
}
...
mod tests {
  ...
  fn maps_letters_without_modifiers() {
    ...
    assert_eq!(n.handle(EventType::KeyPress(Key::KeyA)), Some(KeyEvent::Letter { key: 'a' }));
    assert_eq!(n.handle(EventType::KeyPress(Key::KeyZ)), Some(KeyEvent::Letter { key: 'z' }));
  }

  #[test]
  fn maps_control_keys() {
    let mut n = Normalizer::new();
    assert_eq!(n.handle(EventType::KeyPress(Key::Backspace)), Some(KeyEvent::Backspace));
    assert_eq!(n.handle(EventType::KeyPress(Key::Escape)), Some(KeyEvent::Escape));
    assert_eq!(n.handle(EventType::KeyPress(Key::Space)), Some(KeyEvent::Space));
    assert_eq!(n.handle(EventType::KeyPress(Key::Return)), Some(KeyEvent::Enter));
  }
  ...
  fn filters_keys_while_command_held() {
    ...
    assert_eq!(n.handle(EventType::KeyPress(Key::KeyS)), Some(KeyEvent::Letter { key: 's' }));
  }
